...
void Read_Accel()
{
  ...
  if (i == 6)  // All bytes received?
  {
    ...
    accel[1] = (((int) buff[3]) << 8) | buff[2];  // X axis (internal sensor y axis)
...
// 9DOF Razor IMU SEN-10736 using HMC5883L magnetometer
    ...
    magnetom[0] = -1 * ((((int) buff[0]) << 8) | buff[1]);  // Y axis (internal sensor -x axis)
    magnetom[1] = -1 * ((((int) buff[4]) << 8) | buff[5]);  // X axis (internal sensor -y axis)
...
void Read_Gyro()
{
  ...
  if (i == 6)  // All bytes received?
  {
    ...
    gyro[1] = -1 * ((((int) buff[2]) << 8) | buff[3]);    // X axis (internal sensor -y axis)
